...
export default async function AdminDashboard() {
  ...
  const statsCards = [
    { label: 'إجمالي المستخدمين', value: totalUsers, icon: 'fa-users', color: 'bg-primary-50 text-primary-600' },
    ...
    { label: 'إجمالي الإيرادات (ج.م)', value: `${((totalRevenue._sum.amountPiasters ?? 0) / 100).toLocaleString('ar-EG')}`, icon: 'fa-chart-line', color: 'bg-gold-50 text-gold-600' },
  ]
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="space-y-2">
              ...
              <Link href="/admin/knowledge-base" className="flex items-center gap-2 text-sm text-gray-600 hover:text-primary-700 p-2 rounded-lg hover:bg-gray-50">
                <i className="fa-solid fa-book text-primary-500 w-4" />
                قاعدة المعرفة
              </Link>
            </div>
          </div>
